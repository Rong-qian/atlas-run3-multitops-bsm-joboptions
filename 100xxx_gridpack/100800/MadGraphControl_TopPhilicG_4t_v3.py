...
#--------------------------------------------------------------
### Set lepton filters 
### Filter strategy: we will select events with at least 2 leptons:
### - one of which is leading and the other is subleading: this is achieved using the common filters in Athena
### - also require at least 1 same-sign lepton pair: this is achieved using a custom filter based on LeptonPairFilter

# select at least 1 leading lepton with pT > 25 GeV and |eta| < 2.8 
include("GeneratorFilters/LeptonFilter.py")
LeptonFilter = filtSeq.LeptonFilter
LeptonFilter.Ptcut = 25000.0 #MeV
LeptonFilter.Etacut = 2.8

# select at least 1 subleading lepton with pT > 12 GeV and |eta| < 2.8
include("GeneratorFilters/MultiLeptonFilter.py")
MultiLeptonFilter = filtSeq.MultiLeptonFilter
MultiLeptonFilter.Ptcut = 12000.0 #MeV
MultiLeptonFilter.Etacut = 2.8
MultiLeptonFilter.NLeptons = 2 #minimum

# select at least 1 same-sign lepton pair
if not hasattr(filtSeq, "LeptonPairFilter" ):
   from GeneratorFilters.GeneratorFiltersConf import LeptonPairFilter
   LeptonPairFilter = LeptonPairFilter("LeptonPairFilter")
   filtSeq += LeptonPairFilter

LeptonPairFilter = filtSeq.LeptonPairFilter
# no requirement on the OS pairs
LeptonPairFilter.NSFOS_Max = -1 
LeptonPairFilter.NSFOS_Min = -1
LeptonPairFilter.NOFOS_Max = -1
LeptonPairFilter.NOFOS_Min = -1
LeptonPairFilter.NSFSS_Min = 0 # at least 0 SFSS pairs with NPairSum_Min which will give at least 1 SS pair
LeptonPairFilter.NOFSS_Min = 0 # at least 0 OSSS pairs with NPairSum_Min which will give at least 1 SS pair
LeptonPairFilter.NSFSS_Max = -1 # no requirement on max of SS pairs
LeptonPairFilter.NOFSS_Max = -1 # no requirement on max of SS pairs
# Count number of SS pair 
LeptonPairFilter.UseSFSSInSum = True
LeptonPairFilter.UseOFSSInSum = True
LeptonPairFilter.UseSFOSInSum = False # no requirement on the OS pairs
LeptonPairFilter.UseOFOSInSum = False # no requirement on the OS pairs
# At least >=1 SS pair
LeptonPairFilter.NPairSum_Min = 1 # at least 1 SS pairs
LeptonPairFilter.NPairSum_Max = -1 # at least 1 SS pairs
...
LeptonPairFilter.OnlyMassiveParents = False 
LeptonPairFilter.Ptcut = 12000.0 #MeV
LeptonPairFilter.Etacut = 2.8
LeptonPairFilter.NLeptons_Min = 2
LeptonPairFilter.NLeptons_Max = -1 # No max of leptons 

# Apply the filters
filtSeq.Expression = "(LeptonFilter and MultiLeptonFilter and LeptonPairFilter)"
